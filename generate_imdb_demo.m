...
dbpath = '/scratch/mdimitri/depth_completion/depth_selection/val_selection_cropped'
    ...
    valset = 116; % first 116 images will be used for validation
    
    pathimage = [dbpath '/image/'];
    pathd = [ dbpath '/velodyne_raw/' ];
    patgt = [ dbpath '/groundtruth_depth/' ];
    
    sz = [384,1280]/1; 
    
    seqSize = 1000; % for loading the 1000 val_selection_cropped images
    ...
    for i=1:size(d1,1) % iterate all images in the folder       
        tic();        
        ...
        imdb.images.data(:,:,1:3,i)   = single(img);
